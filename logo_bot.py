        ...
        if r.status_code == 200:
            ...
            for h in items:
                ...
                if len(d) > 2:
                    ...
                    # YENİ: Sembolü de ekledik
                    data[sembol] = {
                        "symbol": sembol, # Kısa Ad (Örn: THYAO)
                        "name": isim,     # Uzun Ad (Örn: Turk Hava Yollari)
                        "logo": logo_url
                    }
...
def get_crypto_metadata():
    ...
    for coin in LISTE_KRIPTO:
        ...
        # YENİ: Symbol alanını ekledik (USD'siz saf hali)
        data[f"{coin}-USD"] = {
            "symbol": coin,      # Kısa Ad (Örn: BTC)
            "name": coin,        # İsim (Örn: BTC - İstenirse uzun isim eklenebilir)
            "logo": logo
        }
...
def get_fon_metadata():
    ...
    for kod, isim in YEDEK_FONLAR.items():
        data[kod] = {"symbol": kod, "name": isim, "logo": FON_ICON}
                ...
                if r.status_code == 200:
                    ...
                    if len(fon_listesi) > 50:
                        for f in fon_listesi:
                            # YENİ: symbol alanı eklendi
                            data[f['FONKODU']] = {
                                "symbol": f['FONKODU'], # Örn: TCD
                                "name": f['FONADI'],    # Örn: Tacirler...
                                "logo": FON_ICON
                            }
...
def get_doviz_altin_metadata():
    ...
    for kur in liste_doviz:
        ...
        if kod in doviz_map:
            info = doviz_map[kod]
            # YENİ: symbol alanı (USD, EUR gibi saf hali)
            data_doviz[kur] = {
                "symbol": kod, 
                "name": info["n"], 
    ...
    data_altin = {}
    for a in altinlar: 
        # YENİ: symbol alanı (İsimle aynı olabilir veya kısaltılabilir)
        data_altin[a] = {"symbol": a, "name": a, "logo": GOLD}
        
    data_altin["Gümüş"] = {"symbol": "Gümüş", "name": "Gümüş", "logo": SILVER}
...
print("✅ TÜM METADATA (SEMBOL, İSİM, LOGO) KAYDEDİLDİ.")
